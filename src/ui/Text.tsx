...
export function TooltipPoint({title, text, color, ...props}: {title: string, text: string, color?: string} & JSX.IntrinsicElements['group']) {
    const [showInfo, setShowInfo] = useState(false)
    return (
        <group {...props} >
            {showInfo ? <Info pos={[1.5,1,0]} title={title} text={text} />:<></>}
            <Html>
                <div className='tooltip' onClick={() => setShowInfo(!showInfo)} >
                    <Tooltip fill={color} />
                </div>
            </Html>
        </group>
